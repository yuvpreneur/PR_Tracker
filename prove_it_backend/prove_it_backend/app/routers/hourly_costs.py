...
def create(payload: CostCreate, db: Database = Depends(get_db), cu=Depends(get_current_user)):
    ...
    log_action(db, user=cu.name, action="CREATE", module="Hourly Costs", record_id=str(cid), detail=f"Set ₹{doc['hourly_cost']}/hr for {doc['emp_id']}")
    return _out(doc)
...
def update(cost_id: int, payload: CostUpdate, db: Database = Depends(get_db), cu=Depends(get_current_user)):
    c = db[collections.HOURLY_COSTS].find_one({"_id": cost_id})
    if not c: raise HTTPException(404, "Record not found")
    ...
    db[collections.HOURLY_COSTS].update_one({"_id": cost_id}, {"$set": patch})
    c = db[collections.HOURLY_COSTS].find_one({"_id": cost_id})
    log_action(db, user=cu.name, action="UPDATE", module="Hourly Costs", record_id=str(cost_id))
    return _out(c)
...
def delete(cost_id: int, db: Database = Depends(get_db), cu=Depends(get_current_user)):
    c = db[collections.HOURLY_COSTS].find_one({"_id": cost_id})
    if not c: raise HTTPException(404, "Record not found")
    db[collections.HOURLY_COSTS].delete_one({"_id": cost_id})
