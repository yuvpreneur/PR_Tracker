...
def _active_projects_by_emp(db: Database, emp_ids: set, org_id) -> dict:
    ...
    if not emp_ids:
        return {}
    active_projects = {p["_id"]: p["name"] for p in db[collections.PROJECTS].find({"status": "In Progress", "org_id": org_id})}
    saved_by_emp = {}
    for p in db[collections.PROJECT_PERMISSIONS].find({"emp_id": {"$in": list(emp_ids)}, "org_id": org_id}):
...
def list_leave(
    ...
    active_by_emp = _active_projects_by_emp(db, {l["emp_id"] for l in rows}, cu.org_id)
...
def summary(db: Database = Depends(get_db), cu=Depends(get_current_user)):
    ...
    mine = my_emp_ids(db, cu)
    rows = list(db[collections.LEAVE].find({"emp_id": {"$in": list(mine)}, "org_id": cu.org_id})) if mine else []
...
def create(payload: LeaveCreate, db: Database = Depends(get_db), cu=Depends(get_current_user)):
    ...
    doc = {"_id": lid, "id": lid, "emp_id": emp_id, "org_id": cu.org_id, **payload.dict(), "days": days, "status": "Pending"}
    doc["from_date"] = doc["from_date"].isoformat()
    doc["to_date"] = doc["to_date"].isoformat()
    db[collections.LEAVE].insert_one(doc)
    log_action(db, user=cu.name, action="CREATE", module="Leave", org_id=cu.org_id, record_id=str(lid), detail=f"{payload.leave_type} leave, {days}d")
    return _out(doc, emp, _active_projects_by_emp(db, {emp_id}, cu.org_id).get(emp_id))
...
def approve(leave_id: int, db: Database = Depends(get_db), cu=Depends(get_current_user)):
    l = get_or_404(db, collections.LEAVE, leave_id, cu.org_id, "Not found")
    if is_own_emp_record(db, cu, l["emp_id"]):
        raise HTTPException(403, "You cannot approve your own leave request")
    db[collections.LEAVE].update_one({"_id": leave_id, "org_id": cu.org_id}, {"$set": {"status": "Approved"}})
    l = db[collections.LEAVE].find_one({"_id": leave_id, "org_id": cu.org_id})
    emp = db[collections.EMPLOYEES].find_one({"_id": l["emp_id"], "org_id": cu.org_id})
    log_action(db, user=cu.name, action="APPROVE", module="Leave", org_id=cu.org_id, record_id=str(leave_id))
    notify(
    ...
    return _out(l, emp, _active_projects_by_emp(db, {l["emp_id"]}, cu.org_id).get(l["emp_id"]))
...
def reject(leave_id: int, payload: RejectPayload, db: Database = Depends(get_db), cu=Depends(get_current_user)):
    ...
    db[collections.LEAVE].update_one({"_id": leave_id, "org_id": cu.org_id}, {"$set": {"status": "Rejected", "decision_reason": payload.reason}})
    l = db[collections.LEAVE].find_one({"_id": leave_id, "org_id": cu.org_id})
    emp = db[collections.EMPLOYEES].find_one({"_id": l["emp_id"], "org_id": cu.org_id})
    log_action(db, user=cu.name, action="REJECT", module="Leave", org_id=cu.org_id, record_id=str(leave_id), detail=payload.reason)
    notify(
